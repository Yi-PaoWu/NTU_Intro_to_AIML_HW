...
class ParticleFilter(InferenceModule):
        ...
        if noisyDistance != None:
            if self.beliefs.totalCount() != 0:
                ...
                #particle = random.choice(self.legalPositions)
                    ...
                    allPossible[particle] = emissionModel[trueDistance]*self.beliefs[particle]
                allPossible.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.legalPositions :
            ...
            #print (newPosDist)
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob*self.beliefs[particle]
        allPossible.normalize()
        for i in range(len(self.particles)):
            self.particles[i] = util.sample(allPossible)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        #print permutation
        print "len = ",len(permutation)
        num = self.numParticles/len(permutation)
        self.particles = []

        for pos in list(permutation) :
            for i in range(num):
                self.particles.append(pos)
    ...
    def observeState(self, gameState):
        ...
        if noisyDistance != None:
            ...
            for g in range(self.numGhosts):
                for p in self.legalPositions :
                    allPossible[p] = 0 
                allPossible[self.getParticleWithGhostInJail(self.getJailPosition(g),g)] = 1  
            for i in range(len(self.particles)):
                self.particles[i] = util.sample(allPossible)
    ...
    def getBeliefDistribution(self):
        ...
        #print "particles = ",self.particles
        allPossible = util.Counter()
        #self.observeState(self.gamestate)
